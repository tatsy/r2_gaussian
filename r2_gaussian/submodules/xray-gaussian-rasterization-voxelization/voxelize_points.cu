...
VoxelizeGaussiansCUDA(const torch::Tensor &means3D,
    ...
    int rendered = 0;
    if (P != 0) {
        rendered = CudaVoxelizer::Voxelizer::forward(geomFunc,
                                                     binningFunc,
                                                     imgFunc,
                                                     P,
                                                     nVoxel_x,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> VoxelizeGaussiansBackwardCUDA(
    ...
    if (P != 0) {
        CudaVoxelizer::Voxelizer::backward(P,
                                           R,
                                           nVoxel_x,
                                           nVoxel_y,
                                           nVoxel_z,
