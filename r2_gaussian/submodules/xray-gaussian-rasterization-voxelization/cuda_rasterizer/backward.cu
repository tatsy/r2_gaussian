...
__global__ void computeCov2DCUDA(int P,
    ...
    if (mode == 0) {  //! parallel beam
        ...
        J = glm::mat3(h_x, 0.0f, 0.0f, 0.0f, h_y, 0.0f, 0, 0, 1.0f);
    } else  //! cone beam
    {
        ...
        J = glm::mat3(h_x / t.z,
                      ...
                      t.x / l,
                      t.y / l,
                      t.z / l);  //! We need last row for computation.
    }
    ...
    if (denom2inv != 0.0f && mu != 0.0f) {
        // exp(*) part gradient
        dL_dhata = denom2inv
                   * (-hatd * hatd * dL_dconic.x + hatb * hatd * dL_dconic.y
                      + (denom - hata * hatd) * dL_dconic.z);  // We remove 2 here because in render we do not *0.5
        dL_dhatd = denom2inv
                   * (-hata * hata * dL_dconic.z + hata * hatb * dL_dconic.y
                      + (denom - hata * hatd) * dL_dconic.x);  // We remove 2 here because in render we do not *0.5
        dL_dhatb =
            denom2inv
            * (2 * hatb * hatd * dL_dconic.x - (denom + 2 * hatb * hatb) * dL_dconic.y + 2 * hata * hatb * dL_dconic.z);

        dL_dhata += pi_mu * ((hatd * hatf - hate * hate) / diamond - hatd * circ_diamond / diamond) * dL_dmu;
        dL_dhatb +=
            pi_mu * ((2 * hatc * hate - 2 * hatf * hatb) / diamond + 2 * hatb * circ_diamond / diamond) * dL_dmu;
    ...
    if (mode == 1) {
        ...
        float dL_dM00 = 2 * (M[0][0] * a + M[0][1] * b + M[0][2] * c) * dL_dhata
                        + (M[1][0] * a + M[1][1] * b + M[1][2] * c) * dL_dhatb
                        + (M[2][0] * a + M[2][1] * b + M[2][2] * c) * dL_dhatc;
        float dL_dM01 = 2 * (M[0][0] * b + M[0][1] * d + M[0][2] * e) * dL_dhata
                        + (M[1][0] * b + M[1][1] * d + M[1][2] * e) * dL_dhatb
                        + (M[2][0] * b + M[2][1] * d + M[2][2] * e) * dL_dhatc;
        float dL_dM02 = 2 * (M[0][0] * c + M[0][1] * e + M[0][2] * f) * dL_dhata
                        + (M[1][0] * c + M[1][1] * e + M[1][2] * f) * dL_dhatb
                        + (M[2][0] * c + M[2][1] * e + M[2][2] * f) * dL_dhatc;

        float dL_dM10 = (M[0][0] * a + M[0][1] * b + M[0][2] * c) * dL_dhatb
                        + 2 * (M[1][0] * a + M[1][1] * b + M[1][2] * c) * dL_dhatd
                        + (M[2][0] * a + M[2][1] * b + M[2][2] * c) * dL_dhate;
        float dL_dM11 = (M[0][0] * b + M[0][1] * d + M[0][2] * e) * dL_dhatb
                        + 2 * (M[1][0] * b + M[1][1] * d + M[1][2] * e) * dL_dhatd
                        + (M[2][0] * b + M[2][1] * d + M[2][2] * e) * dL_dhate;
        float dL_dM12 = (M[0][0] * c + M[0][1] * e + M[0][2] * f) * dL_dhatb
                        + 2 * (M[1][0] * c + M[1][1] * e + M[1][2] * f) * dL_dhatd
                        + (M[2][0] * c + M[2][1] * e + M[2][2] * f) * dL_dhate;

        float dL_dM20 = (M[0][0] * a + M[0][1] * b + M[0][2] * c) * dL_dhatc
                        + (M[1][0] * a + M[1][1] * b + M[1][2] * c) * dL_dhate
                        + 2 * (M[2][0] * a + M[2][1] * b + M[2][2] * c) * dL_dhatf;
        float dL_dM21 = (M[0][0] * b + M[0][1] * d + M[0][2] * e) * dL_dhatc
                        + (M[1][0] * b + M[1][1] * d + M[1][2] * e) * dL_dhate
                        + 2 * (M[2][0] * b + M[2][1] * d + M[2][2] * e) * dL_dhatf;
        float dL_dM22 = (M[0][0] * c + M[0][1] * e + M[0][2] * f) * dL_dhatc
                        + (M[1][0] * c + M[1][1] * e + M[1][2] * f) * dL_dhate
                        + 2 * (M[2][0] * c + M[2][1] * e + M[2][2] * f) * dL_dhatf;
        ...
        float dL_dtx = x_grad_mul
                       * (-h_x * inv_tz2 * dL_dJ02 + (1 / circledcirc - tx * tx * inv_circledcirc3) * dL_dJ20
                          - tx * ty * inv_circledcirc3 * dL_dJ21 - tx * tz * inv_circledcirc3 * dL_dJ22);
        float dL_dty =
            y_grad_mul
            * (-h_y * inv_tz2 * dL_dJ12 - tx * ty * inv_circledcirc3 * dL_dJ20
               + (1 / circledcirc - ty * ty * inv_circledcirc3) * dL_dJ21 - ty * tz * inv_circledcirc3 * dL_dJ22);
        float dL_dtz = -h_x * inv_tz2 * dL_dJ00 + 2 * h_x * tx * inv_tz3 * dL_dJ02 - h_y * inv_tz2 * dL_dJ11
                       + 2 * h_y * ty * inv_tz3 * dL_dJ12 - tx * tz * inv_circledcirc3 * dL_dJ20
                       - ty * tz * inv_circledcirc3 * dL_dJ21
                       + (1 / circledcirc - tz * tz * inv_circledcirc3) * dL_dJ22;
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
        ...
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
            ...
            atomicAdd(&dL_dconic2D[global_id].y,
                      -1.0f * gdx * d.y * dL_dG);  // gs code is 0.5f, which is futher compensated in computecov2D
