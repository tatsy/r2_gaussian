...
__global__ void preprocessCUDA(int P,
    ...
    glm::mat3 M = glm::mat3(1.f / dVoxel_x, 0.0f, 0.0f, 0.0f, 1.f / dVoxel_y, 0.0f, 0.0f, 0.0f, 1.f / dVoxel_z);
    ...
    float3 my_radius = { ceil((3.f * max_scale) / dVoxel_x),
    ...
    if (point_vol.x + my_radius.x < 0 || point_vol.y + my_radius.y < 0 || point_vol.z + my_radius.z < 0
        || point_vol.x - my_radius.x > (float)nVoxel_x || point_vol.y - my_radius.y > (float)nVoxel_y
        || point_vol.z - my_radius.z > (float)nVoxel_z) {
        return;
    }
    ...
    if ((cube_max.x - cube_min.x) * (cube_max.y - cube_min.y) * (cube_max.z - cube_min.z) == 0) return;
...
__global__ void __launch_bounds__(BLOCK3D_X *BLOCK3D_Y *BLOCK3D_Z) renderCUDA(const uint2 *__restrict__ ranges,
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK3D_SIZE) {
        ...
        if (num_done == BLOCK3D_SIZE) break;
        ...
        for (int j = 0; !done && j < min(BLOCK3D_SIZE, toDo); j++) {
            ...
            for (int ch = 0; ch < CHANNELS; ch++) C[ch] += alpha;
...
void FORWARD::render(const dim3 grid,
    ...
    renderCUDA<NUM_CHANNELS><<<grid, block>>>(
        ranges, point_list, nVoxel_x, nVoxel_y, nVoxel_z, means3D_norm, conic_opacity, n_contrib, out_volume);
}
